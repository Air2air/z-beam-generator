...
BATCH_CONFIG = {
    ...
    "multi_subject": {
        ...
        "limit": [1,40] # Range [start_idx, end_idx] to process items by index (or a single number for first N items, None for all subjects)
    },
...
def prompt_for_autonomous_fix_authorization(subject: str, failed_components: list, error_details: str = None) -> bool:
    ...
    while True:
        ...
        if response in ['y', 'yes']:
            ...
            print(f"❌ Declined: Skipping {subject} without fixes")
            return False
        else:
...
def apply_content_quality_fix(content: str, component: str, subject: str, instructions: dict) -> str:
    """Replace placeholder content with material-specific data"""
    try:
        ...
        placeholders_to_replace = {
            ...
            '[material]': subject,
            '{material}': subject,
            'TODO': f"Information about {subject}",
    ...
    for i, subject_info in enumerate(subjects_with_info, 1):
        ...
        if failed_components:
            ...
            if remaining_failed:
                ...
                retry_choice = input("Attempt additional autonomous fixes? (y/n): ").strip().lower()
