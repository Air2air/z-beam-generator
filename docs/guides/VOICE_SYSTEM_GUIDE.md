...
**FORBIDDEN in domain prompts** (prompt catalog `catalog.byPath` entries like `prompts/{domain}/*.txt`):
```
...
**Symptom**: Text like "Use active voice" or "NO conversational tone" in prompt catalog entries `prompts/{domain}/*.txt`
