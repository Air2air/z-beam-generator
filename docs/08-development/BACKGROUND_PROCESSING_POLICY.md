...
**Rationale**: Core Principle #0 (Universal Text Processing Pipeline) - any text still generated by Python must go through QualityEvaluatedGenerator for consistent quality, voice compliance, and learning. Export-time transforms must consume canonical source data rather than regenerating copy.
