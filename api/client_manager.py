...
def create_api_client(provider: str) -> APIClient:
    ...
    if not api_key:
        raise RuntimeError(f"API key missing for {provider}. Set {provider_config['env_var']} environment variable.")
    ...
    for provider_id in providers_to_test:
        try:
            ...
            results[provider_id] = {
                ...
                'error': getattr(response, 'error_message', None) if not response.success else None
            }
