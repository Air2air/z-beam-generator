    ...
    try:
        ...
        for material_name in target_materials:
            ...
            try:
                ...
                if not author_id:
                    logger.error(
                        f"    ❌ FAILED: {material_name} - No author_id in material data"
                    )
                    continue
                ...
                if not author_info:
                    logger.error(
                        f"    ❌ FAILED: {material_name} - Invalid author_id: {author_id}"
                    )
                    continue
                ...
                if result and result.success:
                    ...
                    logger.info(
                        f"    ✅ SUCCESS: {material_name} (Author {author_id}: {author_info['name']}) ({generation_time:.1f}s)"
                    )
                    logger.info(f"    📁 Saved to: {filepath}")

                else:
                    logger.error(
                        f"    ❌ FAILED: {material_name} - {result.error_message if result else 'No result'}"
                    )
        ...
        logger.info(f"⏱️  Total time: {total_time:.1f}s")
        logger.info(
            f"📈 Average time per content: {total_time/max(total_materials, 1):.1f}s"
        )

        if total_materials < 24:
            logger.warning(
                f"⚠️  {24 - total_materials} content files failed generation"
            )
            return 1
        else:
            logger.info("🎉 All content files generated successfully!")
