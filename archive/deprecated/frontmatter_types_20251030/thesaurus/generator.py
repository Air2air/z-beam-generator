...
class ThesaurusFrontmatterGenerator(BaseFrontmatterGenerator):
    ...
    def _load_type_data(self):
        """
        Load thesaurus-specific data from data/thesaurus.yaml
        """
        import yaml
        from pathlib import Path
        
        data_file = Path(__file__).parent.parent.parent.parent.parent / 'data' / 'thesaurus.yaml'
        
        try:
            with open(data_file, 'r') as f:
                data = yaml.safe_load(f)
                self._terms = data.get('terms', {})
                self.logger.info(f"Loaded {len(self._terms)} thesaurus terms from {data_file}")
