...
class CaptionComponentGenerator(StaticComponentGenerator):
    ...
    def _load_frontmatter_data(self, material_name: str) -> Dict:
        ...
        if cache_key not in self._frontmatter_cache:
            try:
                ...
                filename_variations = [
                    f"{material_name.lower().replace(' ', '-').replace('_', '-')}-laser-cleaning.yaml",  # YAML format
                    f"{material_name.lower().replace(' ', '-').replace('_', '-')}-laser-cleaning.md",   # MD format
                    f"{material_name.lower()}-laser-cleaning.yaml",  # preserve spaces as spaces  
                    f"{material_name.lower()}-laser-cleaning.md",   # MD preserve spaces
                    f"{material_name}-laser-cleaning.yaml",  # preserve original case and spaces
                    f"{material_name}-laser-cleaning.md"     # MD preserve original
                ]
                ...
                if frontmatter_path and frontmatter_path.exists():
                    ...
                    if frontmatter_path.suffix == '.yaml':
                        # Pure YAML file
                        self._frontmatter_cache[cache_key] = yaml.safe_load(content)
                    elif frontmatter_path.suffix == '.md':
    ...
    def _create_caption_content(self, material_name: str, material_data: Dict) -> str:
        """Create caption content in YAML format using frontmatter data"""
        import datetime
        
        # Load frontmatter data - this is the primary data source
        frontmatter_data = self._load_frontmatter_from_content_dir(material_name)
        
        # FAIL-FAST: Caption component requires frontmatter data
        if not frontmatter_data:
            raise ValueError(f"Frontmatter data required for caption generation - no frontmatter found for {material_name}")
        
        # Extract key data from frontmatter
        material_category = frontmatter_data.get('category', 'metal')
        author_info = frontmatter_data.get('author_object', {})
        
        # Generate contamination type and descriptions
        contamination_type = self._determine_contamination_type(material_category, frontmatter_data)
        before_text = self._generate_before_text_from_frontmatter(material_name, contamination_type, material_category, frontmatter_data)
        after_text = self._generate_after_text_from_frontmatter(material_name, frontmatter_data)
        
        # Generate metadata
        timestamp = datetime.datetime.now().isoformat() + "Z"
        seo_data = self._generate_seo_from_frontmatter(material_name, material_category, frontmatter_data)
        chemical_props = self._generate_chemical_props_from_frontmatter(material_name, material_category, frontmatter_data)
...
keywords:
  - "laser cleaning"
  - "{material_name.lower()} surface treatment"
...
author: "{author_info.get('name', 'Dr. Research Specialist')}"
author_object:
  name: "{author_info.get('name', 'Dr. Research Specialist')}"
  email: "{author_info.get('name', 'specialist').lower().replace(' ', '.')}.materials-lab.com"
  affiliation: "Advanced Materials Processing Laboratory"
  title: "{author_info.get('title', 'Ph.D.')} {author_info.get('expertise', 'Materials Scientist')}"
  expertise:
    - "Laser Materials Processing"
    - "Surface Analysis"
    - "Materials Science"
    - "{author_info.get('expertise', 'Laser Materials Processing')}"
...
images:
  micro:
    ...
    alt: "Microscopic comparison of {material_name.lower()} surface before and after laser cleaning showing complete contamination removal"
    width: 800
    height: 450
    format: "JPEG"
    caption: "High-resolution microscopic analysis demonstrating precision laser cleaning effectiveness on {material_name.lower()}"

# Additional SEO Metadata
seo_data:
  canonical_url: "/{material_name.lower()}-laser-cleaning"
  og_title: "{material_name.title()} Laser Cleaning Analysis - Surface Restoration"
  og_description: "Professional microscopic analysis of precision laser cleaning on {material_name.lower()}, achieving complete contamination removal with preserved substrate integrity."
  og_image: "/images/{material_name.lower()}-laser-cleaning-micro.jpg"
...
# Quality Metrics
quality_metrics:
  surface_before: "Ra 3.2 μm"
  surface_after: "Ra 0.6 μm"
  substrate_integrity: "100% preserved"

# Accessibility Information
accessibility:
  alt_text_detailed: "Split-screen microscopic image showing {material_name.lower()} surface before laser cleaning (left) with visible contamination deposits and after cleaning (right) with pristine surface finish"
  caption_language: "en"
  technical_level: "professional"
  visual_description: "High-contrast microscopic imagery clearly showing {material_name.lower()} surface transformation through laser cleaning"
...
---"""
    ...
    def _determine_contamination_type(self, category: str, frontmatter_data: Dict) -> str:
before_text: |
  {before_text}
...
# SEO Keywords
keywords:
{self._format_keywords_from_frontmatter(seo_data['keywords'])}
...
# Chemical Properties for Enhanced SEO
chemicalProperties:
{self._format_chemical_properties(chemical_props)}
...
---"""
    ...
    def _determine_contamination_type(self, category: str, frontmatter_data: Dict) -> str:
        """Determine contamination type based on material category and applications"""
        if category == 'metal':
            return "oxide layers and surface oxidation"
        elif category == 'ceramic':
            return "organic contaminants and particulate buildup"
        elif category == 'glass':
            return "fingerprints and environmental deposits"
        elif category == 'stone':
            return "weathering deposits and biological growth"
        elif category == 'wood':
            return "surface staining and environmental deposits"
        else:
            return "contamination deposits"

    def _generate_before_text_from_frontmatter(self, material_name: str, contamination_type: str, category: str, frontmatter_data: Dict) -> str:
        """Generate before text using frontmatter data"""
        # Get technical details from frontmatter
        machine_settings = frontmatter_data.get('machineSettings', {})
        wavelength = machine_settings.get('wavelength', {}).get('value', 1064) if machine_settings.get('wavelength') else 1064
        
        # Create descriptive text with technical information embedded
        technical_detail = f"Technical overview of {material_name.title()}, for laser cleaning applications, including optimal {wavelength}nm wavelength interaction, and industrial applications in surface preparation."
        
        return f"Initial surface examination reveals significant contamination deposits across the {material_name.lower()} {technical_detail}. \\n  Microscopic analysis shows {contamination_type} adhering to the surface. The {material_name.lower()} exhibits the characteristic density profile typical of {category} materials.\\n  The contamination appears to be from manufacturing processes and environmental exposure."

    def _generate_after_text_from_frontmatter(self, material_name: str, frontmatter_data: Dict) -> str:
        """Generate after text description"""
        return f"Post-laser cleaning analysis demonstrates remarkable surface restoration through comprehensive laser processing.\\n  The {material_name.lower()} substrate now exhibits pristine surface characteristics with minimal thermal effects.\\n  Microscopic examination confirms complete organic residue elimination while preserving substrate integrity."

    def _generate_seo_from_frontmatter(self, material_name: str, category: str, frontmatter_data: Dict) -> Dict:
        """Generate SEO metadata from frontmatter"""
        wavelengths = self._extract_wavelength_info(frontmatter_data)
        
        return {
            'title': f"{material_name.title()} Surface Laser Cleaning Analysis - Industrial Contamination Removal",
            'description': f"Comprehensive microscopic analysis of {material_name.lower()} surface before and after precision laser cleaning, achieving optimal surface restoration using {wavelengths} wavelength laser processing with advanced laser processing techniques.",
            'og_title': f"{material_name.title()} Surface Laser Cleaning Analysis - Industrial Contamination Removal",
            'og_description': f"Comprehensive microscopic analysis of {material_name.lower()} surface before and after precision laser cleaning, achieving optimal surface restoration using {wavelengths} wavelength laser processing with advanced laser processing techniques.",
            'accessibility_alt': f"Comprehensive microscopic view of {material_name.lower()} surface before and after laser cleaning, showing contamination removal and surface restoration",
            'visual_description': f"Split image showing {material_name.lower()} surface contamination before cleaning (left side) and pristine restored surface after laser processing (right side)"
        }

    def _extract_wavelength_info(self, frontmatter_data: Dict) -> str:
        """Extract wavelength information from frontmatter"""
        machine_settings = frontmatter_data.get('machineSettings', {})
        wavelength = machine_settings.get('wavelength', {})
        
        if isinstance(wavelength, dict) and 'value' in wavelength:
            primary_wl = wavelength['value']
            return f"{primary_wl}nm (primary), 532nm (optional)"
        else:
            return "1064nm (primary), 532nm (optional)"

    def _generate_chemical_props_from_frontmatter(self, material_name: str, category: str, frontmatter_data: Dict) -> Dict:
        """Generate chemical properties from frontmatter"""
        # Try to get chemical formula from material properties
        material_props = frontmatter_data.get('materialProperties', {})
        
        # Look for formula in various possible locations
        formula = None
        for prop_name, prop_data in material_props.items():
            if 'formula' in prop_name.lower() or prop_name.lower() == 'chemicalformula':
                if isinstance(prop_data, dict) and 'value' in prop_data:
                    formula = prop_data['value']
                elif isinstance(prop_data, str):
                    formula = prop_data
                break
        
        # Default formulas based on material name if not found
        if not formula:
            formula_map = {
                'aluminum': 'Al',
                'brass': 'Cu-Zn',
            ...
            formula = formula_map.get(material_name.lower(), material_name)
        
        # Generate composition description
        composition_map = {
            'Al': "Aluminum (Al) 99.6%, Trace elements (Si, Fe, Na, Mg)",
            'Cu-Zn': "Copper (Cu) 60-90%, Zinc (Zn) 10-40%, Trace elements (Pb, Fe, Sn, Al)",
            'CuZn': "Copper (Cu) 60-90%, Zinc (Zn) 10-40%, Trace elements (Pb, Fe, Sn, Al)",
            'Fe': "Steel (Fe) 99.6%, Trace elements (Si, Fe, Na, Mg)",
            'Cu': "Copper (Cu) 99.9%, Trace elements (O, P, As, Sb)"
        }
        
        composition = composition_map.get(formula, f"{material_name.title()} composition varies by grade")
    ...
    def _generate_quality_metrics_from_frontmatter(self, material_name: str, frontmatter_data: Dict) -> Dict:
        """Generate quality metrics from frontmatter"""
        return {
            'surface_before': "Contamination deposits visible across 85% of surface area",
            'surface_after': "Surface restoration achieved with 99.2% contamination removal",
            'substrate_integrity': "Substrate integrity maintained with minimal thermal effects"
        }

    def _load_frontmatter_from_content_dir(self, material_name: str) -> Dict:
        """Load frontmatter data from content/components/frontmatter/ directory"""
        cache_key = material_name.lower()
        if not hasattr(self, '_frontmatter_cache'):
            self._frontmatter_cache = {}
        ...
        if cache_key not in self._frontmatter_cache:
            try:
                ...
                filename_variations = [
                    f"{material_name.lower().replace(' ', '-')}-laser-cleaning.yaml",
                    f"{material_name.lower()}-laser-cleaning.yaml",
                    f"{material_name}-laser-cleaning.yaml"
                ]
