...
def validate_tags_format(content: str, format_rules: Dict[str, Any] = None) -> List[str]:
    """
...
def validate_tags_content(content: str) -> List[str]:
    ...
    if found_categories < 1:
        errors.append("Tags should include at least one of: laser-cleaning, material-related")
...
def validate_tags_quality(content: str) -> List[str]:
    ...
    technical_tags = sum(1 for tag in tags if any(word in tag for word in ['laser', 'industrial', 'surface', 'precision', 'cleaning']))
    application_tags = sum(1 for tag in tags if any(word in tag for word in ['automotive', 'aerospace', 'manufacturing', 'medical']))
