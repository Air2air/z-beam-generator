...
class TextComponentGenerator(APIComponentGenerator):
    ...
    def generate(
        ...
        try:
            ...
            logger.info(
                f"📝 Generating content for {material_name} using simplified approach"
            )
            ...
            if not result.success:
                logger.warning(
                    f"❌ Content generation failed for {material_name}: {result.error_message}"
                )
    ...
    def _update_frontmatter_with_iterations(
        self,
        content: str,
        score: float,
        ai_result=None,
        ...
        try:
                ...
                if ai_result.details:
                    ...
                    for key, value in ai_result.details.items():
                        ...
                        if key == "sentences" and isinstance(value, list):
                            ...
                            for sentence_data in value[
                                : dynamic_config.get("max_sentence_details", 5)
                                if dynamic_config
                                else 5
                                ...
                                if isinstance(sentence_data, dict):
                                    ai_lines.append(
                                        '      - text: "{}"'.format(
                                            sentence_data.get("text", "").replace(
                                                '"', '\\"'
                                            )
                                        )
                                    )
            ...
            if iteration_history:
                updated_lines.extend(["iteration_history:"])
                ...
                for iter_data in iteration_history:
                    ...
                    if (
                        "deepseek_response" in iter_data
                        and iter_data["deepseek_response"]
                    ):
                        ...
                        updated_lines.append(
                            f'    deepseek_response: "{deepseek_formatted}"'
                        )

                    if iter_data["enhancements_applied"]:
                        enhancements_str = ", ".join(
                            f'"{enh}"' for enh in iter_data["enhancements_applied"]
                        )
                        updated_lines.append(
                            f"    enhancements_applied: [{enhancements_str}]"
                        )
                    else:
