...
def run_content_validation():
    ...
    try:
        ...
        if success:
            logger.info("✅ Persona validation PASSED - Content generation core is validated")
        else:
            logger.error("❌ Persona validation FAILED - Critical content generation issue")
        ...
        for author_id in [1, 2, 3, 4]:
            try:
                ...
                if not persona_config or len(persona_config) == 0:
                    test_passed = False
                    logger.error(f"   ❌ Author {author_id}: Empty persona configuration")
                else:
                    logger.info(f"   ✅ Author {author_id}: Persona configuration loaded")
            except Exception as e:
                test_passed = False
    ...
    try:
        ...
        if not content_dir.exists():
            ...
            if len(content_files) == expected_count:
                logger.info(f"✅ Content files count: {len(content_files)}/{expected_count}")
                ...
                if files_with_content >= 3:
                    logger.info(f"✅ Content quality: {files_with_content}/5 files have substantial content")
                else:
                    logger.warning(f"⚠️  Content quality: Only {files_with_content}/5 files have substantial content")
                    
            else:
                logger.error(f"❌ Content files count: {len(content_files)}/{expected_count}")
                validation_results['content_files'] = False
