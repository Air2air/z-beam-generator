...
def validate_jsonld_structure(content: str, format_rules: Dict[str, Any] = None) -> List[str]:
    """
    ...
    try:
        ...
        # Validate @context
        if '@context' in parsed_json and parsed_json['@context'] != 'https://schema.org':
            errors.append("@context should be 'https://schema.org' for schema.org compliance")
    ...
    try:
        ...
        if json_start != -1 and json_end != -1:
            ...
            if parsed_json.get('@type') == 'Article':
                recommended_fields = ['headline', 'description', 'author', 'datePublished', 'keywords']
                missing_fields = [field for field in recommended_fields if field not in parsed_json]
                if missing_fields:
                    warnings.append(f"Consider adding Article fields: {', '.join(missing_fields)}")
