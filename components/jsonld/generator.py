...
Generates JSON-LD structured data using dynamic frontmatter substitution.
...
class JsonldGenerator:
    """
    JSON-LD generator using schema.org structures.
    ...
    def generate(
        ...
        """Generate JSON-LD using frontmatter data"""
        try:
            ...
            jsonld_content = self._build_jsonld(material_name, frontmatter_data, author_info)
            ...
            logger.error(f"JSON-LD generation failed for {material_name}: {e}")
            return ComponentResult(
                component_type=self.component_type,
                content="",
                success=False,
    ...
    def _build_jsonld(self, material_name: str, frontmatter_data: Dict, author_info: Optional[Dict]) -> str:
        """Build JSON-LD with dynamic substitution"""
...
JsonldComponentGenerator = JsonldGenerator
