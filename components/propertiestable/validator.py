...
def validate_propertiestable_format(content: str, format_rules: Dict[str, Any] = None) -> List[str]:
    """
    ...
    if lines:
        ...
        if len(table_lines) < 3:  # Header + separator + at least one data row
            errors.append("Properties table should have header, separator, and data rows")
...
def validate_propertiestable_data(content: str) -> List[str]:
    ...
    lines = [line.strip() for line in content.split('\n') if line.strip()]
    table_lines = [line for line in lines if '|' in line and not ('|-' in line or '-|' in line)]
        ...
        for row in data_rows:
            ...
            if len(cells) >= 2:  # Property | Value
                ...
                if value in ['N/A', 'TBD', '', 'Unknown']:
                    warnings.append("Consider providing specific data instead of placeholder values")
                    break
    
    # Check for appropriate number of properties
    data_row_count = len([line for line in table_lines if '|' in line and not line.startswith('|--')])
    if data_row_count < 4:  # Less than 4 total rows (including header)
        warnings.append("Consider adding more material properties for comprehensive coverage")
