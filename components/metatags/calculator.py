...
def sanitize_content(content: str) -> str:
    ...
    content = re.sub(r'javascript:', '', content, flags=re.IGNORECASE)  # Remove javascript:
    content = re.sub(r'on\w+\s*=', '', content, flags=re.IGNORECASE)  # Remove event handlers
    content = re.sub(r'drop\s+table[^;]*', '', content, flags=re.IGNORECASE)  # Remove SQL injection
    content = re.sub(r';--', '', content)  # Remove SQL comment injection
    content = re.sub(r'alert\s*\(', '', content, flags=re.IGNORECASE)  # Remove alert() calls
...
class MetatagsCalculator:
    ...
    def __init__(self, frontmatter_data: Dict[str, Any]):
        ...
        self.subject = sanitize_content(frontmatter_data.get('subject', 'Unknown Material'))
    ...
    def generate_seo_keywords(self) -> List[str]:
        ...
        category_keywords = {
            ...
            'ceramic': ['ceramic restoration', 'archaeological conservation', 'heritage preservation'],
            'stone': ['stone conservation', 'monument restoration', 'heritage cleaning'],
            'glass': ['optical cleaning', 'precision optics', 'glass restoration'],
            'composite': ['composite repair', 'advanced materials', 'aerospace applications'],
            'wood': ['wood restoration', 'eco-friendly cleaning', 'cultural heritage'],
            'semiconductor': ['semiconductor processing', 'electronics manufacturing', 'clean room applications']
        }
    ...
    def generate_opengraph_data(self) -> Dict[str, Any]:
        ...
        og_data = [
            {
                "property": "og:title",
                "content": self.calculate_meta_title()
            },
            {
            ...
            },
            {
                ...
                "content": f"{self.subject} laser cleaning"
            }
        ]
    ...
    def generate_advanced_meta_tags(self) -> List[Dict[str, str]]:
        ...
        meta_tags = [
            ...
            {
                "name": "keywords", 
                "content": self.calculate_structured_data_keywords()
            },
            {
                "name": "author",
                "content": sanitize_content(self.frontmatter.get('author', 'Z-Beam Technical Team'))
            },
            {
                "name": "category",
...
def calculate_metatags_for_material(material_file_path: str) -> str:
    ...
    return yaml.dump(metatags_data, default_flow_style=False, sort_keys=False, allow_unicode=True)
