...
class MetatagsComponentGenerator(FrontmatterComponentGenerator):
    ...
    def _extract_from_frontmatter(
        ...
        try:
            ...
            if schema_path.exists():
                with open(schema_path, "r", encoding="utf-8") as f:
                    ...
                    material_profile = schema.get("materialProfile", {}).get(
                        "profile", {}
                    )
                    schema_fields = {
        ...
        if example_tags:
            ...
            meta_tags = self._build_from_example(
                frontmatter_data, example_tags, material_name
            )
        elif schema_fields:
            # Use schema structure
            meta_tags = self._build_from_schema(
                frontmatter_data, schema_fields, material_name
            )
        else:
            # FAIL-FAST: No fallback allowed - system must have required schema or example
            raise Exception(
                f"No schema or example provided for metatags generation of {material_name} - fail-fast architecture requires explicit configuration"
            )
    ...
    def _build_from_example(
        self, frontmatter_data: Dict, example_tags: Dict, material_name: str
    ) -> list:
        """Build meta tags using example structure as template"""
        meta_tags = []

        for tag_name, example_content in example_tags.items():
            content = self._generate_meta_content(
                frontmatter_data, tag_name, material_name, example_content
            )
    ...
    def _build_from_schema(
        self, frontmatter_data: Dict, schema_fields: Dict, material_name: str
    ) -> list:
        ...
        keywords = self._generate_keywords_from_schema(
            frontmatter_data, schema_fields, material_name
        )
    ...
    def _generate_meta_content(
        ...
        if tag_name in ["description", "og:description", "twitter:description"]:
            ...
            extracted = self._get_field(
                frontmatter_data, [tag_name.replace(":", ".")], ""
            )
            return extracted if extracted else example_content

    def _generate_keywords_from_schema(
        self, frontmatter_data: Dict, schema_fields: Dict, material_name: str
    ) -> list:
