...
def validate_metatags_yaml(
    ...
    try:
        ...
        if "meta_tags" in parsed_yaml and not isinstance(
            parsed_yaml["meta_tags"], list
        ):
            errors.append("meta_tags must be a list")
        if "opengraph" in parsed_yaml and not isinstance(
            parsed_yaml["opengraph"], list
        ):
        ...
        if yaml_end != -1:
            ...
            if parsed_yaml and "meta_tags" in parsed_yaml:
                ...
                if isinstance(meta_tags, list):
                    ...
                    for tag in meta_tags:
                        if isinstance(tag, dict) and tag.get("name") == "description":
                            ...
                            if len(desc_content) < 120:
                                warnings.append(
                                    "Meta description may be too short (recommended: 150-160 characters)"
                                )
                            elif len(desc_content) > 170:
                                warnings.append(
                                    "Meta description may be too long (recommended: 150-160 characters)"
                                )
                    ...
                    missing_tags = [
                        tag for tag in essential_tags if tag not in tag_names
                    ]
                    if missing_tags:
                        warnings.append(
                            f"Consider adding meta tags: {', '.join(missing_tags)}"
                        )
