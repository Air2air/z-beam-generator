...
class FailFastContentGenerator:
    ...
    def _call_api_with_validation(self, api_client, user_prompt: str, system_prompt: str = None):
        ...
        try:
            ...
            if hasattr(api_client, 'generate_simple'):
                # For generate_simple, combine system and user prompts
                combined_prompt = user_prompt
                if system_prompt:
                    combined_prompt = f"{system_prompt}\n\n{user_prompt}"
                response = api_client.generate_simple(combined_prompt)
                ...
                if isinstance(response, str):
                    ...
                    class SimpleResponseWrapper:
                        def __init__(self, content, prompt_tokens, system_tokens=0):
                            self.content = content
                            self.success = True
                            self.request_id = "simple_api_call"
                            self.response_time = 0.0
                            self.token_count = len(content.split())
                            self.prompt_tokens = prompt_tokens + system_tokens
                            self.completion_tokens = len(content.split())
                            self.model_used = "unknown"
                            self.retry_count = 0
                    response = SimpleResponseWrapper(response, len(user_prompt.split()), len(system_prompt.split()) if system_prompt else 0)
            else:
                # Use GenerationRequest for newer API clients
                from api.client import GenerationRequest
                request = GenerationRequest(prompt=user_prompt, system_prompt=system_prompt)
    ...
    def _build_api_prompt(self, subject: str, formula: str, author_name: str, 
                         ...
                         base_config: Dict, persona_config: Dict, formatting_config: Dict,
                         author_info: Optional[Dict] = None) -> Tuple[str, str]:
        """Build API prompt from configurations - NO FALLBACK CONTENT.
        
        Returns:
            Tuple of (system_prompt, user_prompt) where:
            - system_prompt contains all guidance, persona, and formatting instructions
            - user_prompt contains material context and specific generation request
        """
        try:
            # SYSTEM PROMPT: All guidance, persona, and formatting instructions
            system_parts = []
            ...
            system_parts.append("PRIMARY CONTENT GUIDANCE:")
            if isinstance(overall_subject, list):
                ...
                system_parts.append(f"- {overall_subject}")
            else:
            ...
            if language_patterns:
                ...
                if signature_phrases:
                    system_parts.append("SECONDARY - LANGUAGE STYLE:")
                    system_parts.extend([f"- {phrase}" for phrase in signature_phrases[:3]])
                    system_parts.append("")
            
            # Add writing style guidance - SECONDARY
            if writing_style:
                system_parts.append("SECONDARY - WRITING STYLE:")
            ...
            if formatting_config:
                ...
                if markdown_formatting:
                    system_parts.append("SECONDARY - FORMATTING REQUIREMENTS:")
                    
                    # Headers
                    headers = markdown_formatting.get('headers', {})
                    if headers:
                        system_parts.append("- Headers: " + headers.get('main_title', '# for main title'))
                        system_parts.append("- Sections: " + headers.get('section_headers', '## for sections'))
                    
                    # Emphasis
                    emphasis = markdown_formatting.get('emphasis', {})
                    if emphasis:
                        system_parts.append("- Bold: " + emphasis.get('critical_info', '**bold** for key information'))
                        system_parts.append("- Formulas: " + emphasis.get('formulas', 'simple notation'))
                    
                    # Lists
                    lists = markdown_formatting.get('lists', {})
                    if lists:
                        system_parts.append("- Lists: " + lists.get('primary_style', 'numbered lists'))
            ...
            if content_structure:
                ...
                if required_sections:
                    system_parts.append("CONTENT STRUCTURE:")
                    system_parts.extend([f"- {section}" for section in required_sections])
                    system_parts.append("")
            
            # Add application focus if available
            app_focus = base_config.get('application_focus', {})
            if app_focus and country_lower in app_focus:
                system_parts.append(f"APPLICATION FOCUS: {app_focus[country_lower]}")
                system_parts.append("")
            ...
            # USER PROMPT: Material context and specific generation request
            user_parts = [
            ...
            if frontmatter_data:
                user_parts.append("MATERIAL CONTEXT:")
                
                # Chemical properties
                chem_props = frontmatter_data.get('chemicalProperties', {})
                if chem_props:
                    user_parts.append(f"- Chemical Formula: {chem_props.get('formula', formula)}")
                    if 'symbol' in chem_props:
                        user_parts.append(f"- Material Symbol: {chem_props['symbol']}")
                    if 'materialType' in chem_props:
                        user_parts.append(f"- Material Type: {chem_props['materialType']}")
                
                # Physical properties
                properties = frontmatter_data.get('properties', {})
                if properties:
                    if 'density' in properties:
                        user_parts.append(f"- Density: {properties['density']}")
                    if 'thermalConductivity' in properties:
                        user_parts.append(f"- Thermal Conductivity: {properties['thermalConductivity']}")
                    if 'meltingPoint' in properties:
                        user_parts.append(f"- Melting Point: {properties['meltingPoint']}")
                
                # Category and technical specs
                if 'category' in frontmatter_data:
                    user_parts.append(f"- Category: {frontmatter_data['category'].title()}")
                
                tech_specs = frontmatter_data.get('technicalSpecifications', {})
                if tech_specs:
                    if 'tensileStrength' in tech_specs:
                        user_parts.append(f"- Tensile Strength: {tech_specs['tensileStrength']}")
    ...
    def _format_api_response_with_metadata(self, response: str, subject: str, author_name: str,
                           ...
                           ai_detection_metadata: Optional[Dict] = None) -> str:
        ...
        try:
            ...
            if ai_detection_metadata:
                frontmatter_lines.extend([
                    "winston_iterations:",
                    f"  enabled: {ai_detection_metadata.get('winston_enabled', False)}",
                    f"  total_iterations: {ai_detection_metadata.get('winston_iterations', 0)}",
                    f"  target_score: {ai_detection_metadata.get('winston_target_score', 'N/A')}",
                    f"  content_improved: {ai_detection_metadata.get('content_improved', False)}",
                ])
                
                # Add iteration history if available
                if 'winston_history' in ai_detection_metadata and ai_detection_metadata['winston_history']:
                    frontmatter_lines.append("  iteration_history:")
                    for iteration in ai_detection_metadata['winston_history']:
                        frontmatter_lines.extend([
