"""
Image URL handler for Z-Beam Generator components.

Provides standardized methods for generat        if not isinstance(data, dict) or "images" not in data:
            return dataand normalizing image URLs.
"""
...
class ImageHandler:
    ...
    def format_image_url(subject: str, image_type: str, custom_slug: Optional[str] = None) -> str:
        ...
        if image_type in ["hero", "process", "equipment", "application"]:
            return f"/images/{slug}-laser-cleaning-{image_type}.jpg"
        else:
            return f"/images/{slug}-{image_type}.jpg"
