...
try:
    from materials.research.material_property_research_system import MaterialPropertyResearchSystem
...
class PropertyValueResearcher:
    ...
    def _materials_yaml_lookup(self, 
        ...
        try:
            from materials.data.materials import get_material_data, get_material_by_name
            from materials.utils.unit_extractor import extract_unit_from_range, extract_numeric_from_range
    ...
    def discover_all_material_properties(self, material_name: str, material_category: str = None) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            from shared.config.settings import GLOBAL_OPERATIONAL_CONFIG
            ...
            try:
                ...
                self.logger.error(f"❌ JSON Parse Error for {material_name}")
                self.logger.error(f"Error: {e}")
                self.logger.error(f"Response length: {len(cleaned_content)} chars")
                self.logger.error(f"First 500 chars: {cleaned_content[:500]}")
                self.logger.error(f"Last 500 chars: {cleaned_content[-500:]}")
                self.logger.error(f"Around error position (char {e.pos if hasattr(e, 'pos') else 'unknown'}): {cleaned_content[max(0, (e.pos if hasattr(e, 'pos') else 0) - 100):(e.pos if hasattr(e, 'pos') else 0) + 100]}")
