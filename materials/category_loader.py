...
class CategoryDataLoader:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        self.categories_dir = self.materials_data_dir / 'categories'
        self.legacy_file = self.materials_data_dir / 'Categories.yaml'
        
        # Determine data source - ALWAYS use legacy file for now (split files incomplete)
        # TODO: Once split files have category_ranges, switch back to split file logic
        self.use_split_files = False  # Force legacy file usage
        
        if not self.legacy_file.exists():
            raise ConfigurationError(
                f"Categories.yaml not found at: {self.legacy_file}\n"
                f"Per GROK_INSTRUCTIONS.md: No fallbacks allowed."
            )
        
        logger.debug("CategoryDataLoader using: materials/data/Categories.yaml")
