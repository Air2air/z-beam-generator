...
class AIDetectionProvider:
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "config": {
                k: "***" if "key" in k.lower() else v for k, v in self.config.items()
            },
        }
...
class AIDetectionOptimizationService(BaseService):
    ...
    def __init__(self, config: ServiceConfiguration):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=config.settings.get("max_workers", 4)
        )
        ...
        self.ai_config.improvement_threshold = config.settings.get(
            "improvement_threshold", 3.0
        )
        self.ai_config.human_threshold = config.settings.get("human_threshold", 75.0)
        self.ai_config.min_text_length_winston = config.settings.get(
            "min_text_length_winston", 300
        )
        self.ai_config.short_content_threshold = config.settings.get(
            "short_content_threshold", 400
        )
        self.ai_config.min_content_length = config.settings.get(
            "min_content_length", 150
        )
        self.ai_config.fallback_score_first_iteration = config.settings.get(
            "fallback_score_first_iteration", 60.0
        )
        self.ai_config.fallback_score_short_content = config.settings.get(
            "fallback_score_short_content", 55.0
        )
        self.ai_config.fallback_score_very_short = config.settings.get(
            "fallback_score_very_short", 40.0
        )
        self.ai_config.fallback_score_error = config.settings.get(
            "fallback_score_error", 50.0
        )
        self.status_update_interval = config.settings.get("status_update_interval", 10)
        self.iteration_status_frequency = config.settings.get(
            "iteration_status_frequency", 5
        )
        self.ai_config.word_count_tolerance = config.settings.get(
            "word_count_tolerance", 1.5
        )
        self.ai_config.winston_timeout_cap = config.settings.get(
            "winston_timeout_cap", 15
        )
        ...
        self.ai_config.winston_human_range = config.settings.get(
            "winston_human_range", (70, 100)
        )
        self.ai_config.winston_unclear_range = config.settings.get(
            "winston_unclear_range", (30, 70)
        )
        self.ai_config.winston_ai_range = config.settings.get(
            "winston_ai_range", (0, 30)
        )
        self.ai_config.min_iterations_before_exit = config.settings.get(
            "min_iterations_before_exit", 3
        )
        self.ai_config.early_exit_score_threshold = config.settings.get(
            "early_exit_score_threshold", 10
        )
        self.ai_config.deepseek_optimization_enabled = config.settings.get(
            "deepseek_optimization_enabled", True
        )
        self.ai_config.config_backup_enabled = config.settings.get(
            "config_backup_enabled", True
        )
        self.ai_config.enable_detailed_logging = config.settings.get(
            "enable_detailed_logging", True
        )
        self.ai_config.max_sentence_details = config.settings.get(
            "max_sentence_details", 5
        )
        ...
        **kwargs,
        ...
        if use_cache:
            cache_key = self._get_cache_key(
                content, providers or list(self.providers.keys())
            )
        ...
        raise AIDetectionProviderError(
            f"All AI detection providers failed. Last error: {last_error}"
        )
    ...
    async def batch_detect_ai_content(
        ...
        for content in request.contents:
            task = self.detect_ai_content(
                content=content, providers=request.providers, **(request.options or {})
            )
            tasks.append(task)
        ...
        summary = {
            ...
            "detection_rate": detected_count / len(successful_results)
            if successful_results
            else 0,
        ...
        return BatchDetectionResult(
            results=successful_results, summary=summary, processing_time=processing_time
        )
    ...
    def optimize_detection_thresholds(
        self, historical_results: List[AIDetectionResult], target_accuracy: float = 0.9
    ) -> Dict[str, float]:
        """
        ...
        optimal_detection_threshold = sorted(scores)[
            int(len(scores) * (1 - target_accuracy))
        ]
        optimal_confidence_threshold = sorted(confidences)[
            int(len(confidences) * (1 - target_accuracy))
        ]
