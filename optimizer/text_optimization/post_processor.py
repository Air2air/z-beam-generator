...
def _standardize_technical_terms(content: str) -> str:
    ...
    content = re.sub(
        r"\bclass\s*4\s*laser", "Class 4 laser", content, flags=re.IGNORECASE
    )
    ...
    content = re.sub(
        r"\blaser\s*cleaning\b", "laser cleaning", content, flags=re.IGNORECASE
    )
    content = re.sub(
        r"\bsurface\s*contamination\b",
...
def _enhance_material_context(
    content: str, material_name: str, material_data: dict
) -> str:
    ...
    if material_data and "density" in material_data:
        ...
        content = re.sub(
            r"density[:\s]*(\d+\.?\d*)\s*g/cm3?",
            f"density of {density} g/cm³",
            content,
            flags=re.IGNORECASE,
        )
...
def _fix_grammar_and_style(content: str) -> str:
    ...
    content = re.sub(
        r"\ban\s+([bcdfghjklmnpqrstvwxyz])", r"a \1", content, flags=re.IGNORECASE
    )

    # Fix common verb tense issues in technical writing
    content = re.sub(
        r"\blaser cleaning provide\b",
        "laser cleaning provides",
        content,
        flags=re.IGNORECASE,
    )
    content = re.sub(
        r"\blaser cleaning offer\b",
    ...
    content = re.sub(
        r"\bvery effective\b", "highly effective", content, flags=re.IGNORECASE
    )
    content = re.sub(
        r"\bvery efficient\b", "highly efficient", content, flags=re.IGNORECASE
    )
...
def _apply_country_style(content: str, author_info: dict) -> str:
    ...
    if country in ["uk", "united kingdom", "britain", "england"]:
        ...
        if not re.search(
            r"\b(firstly|initially|furthermore|moreover)\b", content, re.IGNORECASE
        ):
