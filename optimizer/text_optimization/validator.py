...
def validate_content_format(
    ...
    if len(paragraphs) < 3:
        errors.append(
            f"Content needs better structure (found {len(paragraphs)} paragraphs, minimum 3)"
        )
    ...
    if len(sentences) < 10:
        errors.append(
            f"Content needs more detail (found {len(sentences)} sentences, minimum 10)"
        )
...
def validate_content_technical(
    content: str, material_data: Dict[str, Any] = None
) -> List[str]:
    """
    ...
    for term_pattern in required_terms:
        if not re.search(term_pattern, content_lower):
            missing_terms.append(
                term_pattern.replace(r"\s*", " ").replace(r"(", "").replace(r")", "")
            )
    ...
    found_params = sum(
        1 for param in technical_params if re.search(param, content_lower)
    )
    if found_params < 2:
        errors.append(
            "Content should include more specific technical parameters (wavelength, pulse duration, power, etc.)"
        )
    ...
    if material_data:
        ...
        if material_name and material_name not in content_lower:
            errors.append(
                f"Content should specifically mention the material: {material_name}"
            )

        material_formula = material_data.get("chemical_formula", "").lower()
        if (
            material_formula
            and len(material_formula) > 2
            and material_formula not in content_lower
        ):
            errors.append(
                f"Content should include material formula: {material_formula}"
            )
...
def validate_content_structure(content: str) -> List[str]:
    ...
    if not any(
        term in first_paragraph.lower()
        for term in ["laser cleaning", "material", "surface"]
    ):
    ...
    if not re.search(
        r"advantage|benefit|superior|efficient|effective", content.lower()
    ):
...
def validate_content_quality(content: str) -> List[str]:
    ...
    technical_count = sum(
        1 for indicator in technical_indicators if indicator in content.lower()
    )
    ...
    passive_count = sum(
        len(re.findall(pattern, content.lower())) for pattern in passive_patterns
    )
...
def validate_human_writing_score(
    content: str, author_info: Dict[str, Any] = None
) -> List[str]:
    """
    ...
    if found_markers:
        errors.append(
            f"Content contains AI-generated markers: {', '.join(found_markers[:3])}"
        )
    ...
    repeated_starts = [
        pattern for pattern in set(start_patterns) if start_patterns.count(pattern) > 2
    ]

    if repeated_starts:
        errors.append(
            f"Repetitive sentence structures detected: {', '.join(repeated_starts[:2])}"
        )
    ...
    formal_count = sum(
        1 for pattern in overly_formal if re.search(pattern, content_lower)
    )
    if formal_count > 1:
    ...
    if synonym_variety < 2:
        errors.append(
            "Limited vocabulary variation - consider using more diverse terminology"
        )

    return errors


def validate_author_country_style(
    content: str, author_info: Dict[str, Any] = None
) -> List[str]:
    """
    ...
    if country in ["uk", "united kingdom", "britain", "england"]:
        ...
        for us_pattern, uk_form in us_spellings:
            if re.search(us_pattern, content_lower):
                found_us.append(
                    f"{us_pattern.replace(r'\\b', '')} (should be {uk_form})"
                )
        ...
        for uk_pattern, us_form in uk_spellings:
            if re.search(uk_pattern, content_lower):
                found_uk.append(
                    f"{uk_pattern.replace(r'\\b', '')} (should be {us_form})"
                )
        ...
        if len(content.split(".")) < 8:
            errors.append(
                "German technical writing typically includes more detailed explanations"
            )

        # Check for systematic structure
        if not re.search(
            r"(firstly|initially|furthermore|moreover|finally)", content_lower
        ):
            errors.append(
                "German technical style benefits from systematic progression indicators"
            )
        ...
        if precision_count < 2:
            errors.append(
                "Japanese technical writing typically emphasizes precision and methodology"
            )

    return errors


def validate_article_variation(
    content: str, previous_articles: List[str] = None
) -> List[str]:
    """
    ...
    for article in previous_articles:
        ...
        for pattern in intro_patterns:
            if re.search(pattern, content_intro) and re.search(pattern, article_intro):
                errors.append(
                    f"Introduction too similar to previous articles: {pattern.replace('^', '').replace(r'\\b', '')}"
                )
                break
    ...
    for article in previous_articles:
        ...
        if len(common_structures) > len(content_structures) * 0.3:
            errors.append(
                "Article structure too similar to previous content - vary sentence beginnings"
            )
            break
    ...
    for article in previous_articles:
        ...
        if len(common_focuses) == len(content_focuses) and len(content_focuses) > 2:
            errors.append(
                "Technical focus identical to previous article - vary emphasis areas"
            )
            break
...
def validate_content_comprehensive(
    content: str,
    ...
    results["country_style_errors"] = validate_author_country_style(
        content, author_info
    )
...
def get_validation_summary(validation_results: Dict[str, List[str]]) -> Dict[str, int]:
    ...
    """
    return {category: len(errors) for category, errors in validation_results.items()}
