...
class WinstonAnalyzer:
    ...
    def analyze_improvement_needs(
        ...
        if score < 40:
            # Critical scores: Need aggressive multi-layered improvements
            analysis["needs_improvement"] = True
            analysis["priority_level"] = "critical"
            analysis["target_sections"] = self._get_critical_targets(
                winston_result, content
            )
        elif score < 60:
            # Moderate scores: Focus on authenticity enhancements
            analysis["needs_improvement"] = True
            analysis["priority_level"] = "high"
            analysis["target_sections"] = self._get_authenticity_targets(
                winston_result, content
            )
        elif score < 80:
            # Good scores: Refine with advanced techniques
            analysis["needs_improvement"] = True
            analysis["priority_level"] = "medium"
            analysis["target_sections"] = self._get_refinement_targets(
                winston_result, content
            )
        elif score < target_score:
            # Below target but not critically low: Minor fine-tuning
            analysis["needs_improvement"] = True
            analysis["priority_level"] = "low"
            analysis["target_sections"] = self._get_minor_targets(
                winston_result, content
            )

        # Analyze content for improvement potential
        analysis["content_analysis"][
            "has_improvement_potential"
        ] = self._analyze_content_potential(content, winston_result)

        logger.info(
            f"📊 Winston Analysis: Score {score}, Priority: {analysis['priority_level']}, "
            f"Targets: {len(analysis['target_sections'])} sections"
        )

        return analysis

    def _get_critical_targets(
        self, winston_result: Dict[str, Any], content: str
    ) -> List[str]:
        ...
        targets = [
            ...
            "cognitive_variability",  # New: Cognitive authenticity
            "human_authenticity_enhancements",  # New: Authenticity markers
        ]
        ...
        return targets[:4]  # Allow more targets for critical cases

    def _get_authenticity_targets(
        self, winston_result: Dict[str, Any], content: str
    ) -> List[str]:
        ...
        targets = [
            ...
            "personal_touch",
            "cultural_humanization",  # New: Cultural adaptation
            "conversational_flow",  # New: Natural flow
        ]
    ...
    def _get_refinement_targets(
        ...
        targets = [
            ...
            "detection_response",
            "content_transformation_rules",  # New: Advanced transformation
            "iteration_refinement_mechanism",  # New: Refinement techniques
        ]
    ...
    def _get_minor_targets(
        ...
        targets = [
            ...
            "ai_detection_focus",  # New: Detection focus
            "detection_response",  # New: Response optimization
        ]
    ...
    def _analyze_content_potential(
        self, content: str, winston_result: Dict[str, Any]
    ) -> bool:
