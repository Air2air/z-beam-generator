...
def check_environment() -> Dict[str, Any]:
    ...
    for provider_id, config in API_PROVIDERS.items():
        ...
        if not api_key:
            results['critical_issues'].append(f"Missing API key for {config['name']} ({config['env_var']})")
    ...
    try:
        ...
        for component, config in components_config.items():
            ...
            if missing_fields:
                results['warnings'].append(f"Component {component} missing fields: {missing_fields}")
...
def format_environment_report(results: Dict[str, Any]) -> str:
    ...
    if 'error' in results['configuration']:
        ...
        lines.append(f"  ✅ Components loaded: {results['configuration']['components_loaded']}")
        lines.append(f"  ✅ Enabled components: {results['configuration']['enabled_components']}")
    ...
    try:
        ...
        for component_name, config in components_config.items():
            ...
            if config.get('data_provider') not in ['API', 'static', 'hybrid']:
                component_result['issues'].append(f"Invalid data_provider: {config.get('data_provider')}")
            
            if config.get('api_provider') and config.get('api_provider') not in API_PROVIDERS:
                component_result['issues'].append(f"Invalid api_provider: {config.get('api_provider')}")
            
            # Check component directory exists
            component_dir = Path(f"components/{component_name}")
            if not component_dir.exists():
                component_result['issues'].append(f"Component directory missing: {component_dir}")
        ...
        results['configuration_errors'].append(f"Failed to load component configuration: {e}")
