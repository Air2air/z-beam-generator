...
class AIDetectionOptimizationService(BaseService):
    ...
    def __init__(self, config: ServiceConfiguration):
        ...
        self.config.target_score = config.settings.get("target_score", 65.0)
        self.config.max_iterations = config.settings.get("max_iterations", 5)
        self.config.improvement_threshold = config.settings.get("improvement_threshold", 3.0)
        self.config.human_threshold = config.settings.get("human_threshold", 75.0)
        self.config.min_text_length_winston = config.settings.get("min_text_length_winston", 300)
        self.config.short_content_threshold = config.settings.get("short_content_threshold", 400)
        self.config.min_content_length = config.settings.get("min_content_length", 150)
        self.config.fallback_score_first_iteration = config.settings.get("fallback_score_first_iteration", 60.0)
        self.config.fallback_score_short_content = config.settings.get("fallback_score_short_content", 55.0)
        self.config.fallback_score_very_short = config.settings.get("fallback_score_very_short", 40.0)
        self.config.fallback_score_error = config.settings.get("fallback_score_error", 50.0)
        self.config.status_update_interval = config.settings.get("status_update_interval", 10)
        self.config.iteration_status_frequency = config.settings.get("iteration_status_frequency", 5)
        self.config.word_count_tolerance = config.settings.get("word_count_tolerance", 1.5)
        self.config.winston_timeout_cap = config.settings.get("winston_timeout_cap", 15)
        self.config.max_tokens = config.settings.get("max_tokens", 3000)
        self.config.retry_delay = config.settings.get("retry_delay", 0.5)
        self.config.winston_human_range = config.settings.get("winston_human_range", (70, 100))
        self.config.winston_unclear_range = config.settings.get("winston_unclear_range", (30, 70))
        self.config.winston_ai_range = config.settings.get("winston_ai_range", (0, 30))
        self.config.min_iterations_before_exit = config.settings.get("min_iterations_before_exit", 3)
        self.config.early_exit_score_threshold = config.settings.get("early_exit_score_threshold", 10)
        self.config.deepseek_optimization_enabled = config.settings.get("deepseek_optimization_enabled", True)
        self.config.config_backup_enabled = config.settings.get("config_backup_enabled", True)
        self.config.enable_detailed_logging = config.settings.get("enable_detailed_logging", True)
        self.config.max_sentence_details = config.settings.get("max_sentence_details", 5)
    ...
    async def analyze_text(self, content: str, **kwargs) -> AIDetectionResult:
        """
        Analyze text for AI detection (backward compatibility method).
        ...
        Returns:
            AIDetectionResult: Detection result
        """
        return await self.detect_ai_content(content, **kwargs)
