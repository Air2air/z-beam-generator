...
from shared.validation.errors import MaterialDataError, GenerationError
...
class MaterialFrontmatterGenerator(BaseFrontmatterGenerator):
    ...
    def _build_frontmatter_data(
        ...
        try:
            ...
            # The legacy generator returns ComponentResult with YAML string in content
            # We need to parse it back to dict for base_generator._save_frontmatter()
            import yaml
...
class StreamlinedFrontmatterGeneratorAlias(MaterialFrontmatterGenerator):
    ...
    Allows code like:
        from export.types.material.generator import StreamlinedFrontmatterGenerator
