...
class MockAPIClient:
    ...
    def __init__(self, provider_name: str = "mock", api_key: str = "test_key", simulate_failures: bool = False):
    ...
    def _apply_provider_optimizations(self, request: GenerationRequest) -> GenerationRequest:
    ...
    def _detect_component_type(self, prompt: str) -> str:
        ...
        patterns = {
            ...
            'propertiestable': ['properties table', 'specifications', 'characteristics']
        }
    ...
    def generate_with_retry(self, request: GenerationRequest, max_retries: int = 3) -> APIResponse:
    ...
    def _generate_content_from_prompt(self, prompt: str) -> str:
        ...
        for material, keywords in materials.items():
            if any(keyword in prompt_lower for keyword in keywords) and "laser" in prompt_lower:
                return getattr(self, f"_generate_{material}_content")()

        # Generic laser cleaning content
        if "laser" in prompt_lower and ("cleaning" in prompt_lower or "removal" in prompt_lower):
