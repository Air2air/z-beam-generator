...
# Batch Subtitle Regeneration Script
...
echo "=================================================="
echo "📝 BATCH SUBTITLE REGENERATION"
echo "=================================================="
echo "Total materials: $TOTAL"
echo "Target word range: 21-63 words (50% of description)"
echo ""
...
for i in "${!MATERIALS[@]}"; do
    ...
    if python3 run.py --subtitle "$MATERIAL" --skip-integrity-check 2>&1 | grep -q "✅ Subtitle generated"; then
        WORD_COUNT=$(python3 run.py --subtitle "$MATERIAL" --skip-integrity-check 2>&1 | grep "Word count:" | tail -1 | awk '{print $3}')
