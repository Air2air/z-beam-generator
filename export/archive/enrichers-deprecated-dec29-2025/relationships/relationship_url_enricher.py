...
This enricher:
1. Builds an index of all frontmatter files (id → full_path mapping)
...
class RelationshipURLEnricher(BaseEnricher):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.index: Dict[str, str] = {}  # id → full_path mapping
    ...
    def enrich(self, frontmatter: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"🔗 RelationshipURLEnricher.enrich() called for {item_id}")
        
        try:
            # Build index on first call (after frontmatter files exist)
            if not self.index_built:
                print(f"🔗 Building relationship URL index...")
                self._build_index()
                print(f"📊 Index built: {len(self.index)} items")
                # Debug: Show sample entries
                if 'pahs-compound' in self.index:
                    print(f"  ✅ pahs-compound → {self.index['pahs-compound']}")
                if 'fire-damage-contamination' in self.index:
                    print(f"  ✅ fire-damage-contamination → {self.index['fire-damage-contamination']}")
            
            print(f"  Step 1: index_built={self.index_built}, index size={len(self.index)}")
            
            # Debug: show what's in index for pahs-compound
            if 'pahs-compound' in self.index:
                print(f"  Index has pahs-compound → {self.index['pahs-compound']}")
            else:
                print(f"  ⚠️  pahs-compound NOT in index!")
            ...
            if 'id' in frontmatter and 'full_path' in frontmatter:
                ...
                if current_id not in self.index or self.index[current_id] != current_path:
                    ...
                    print(f"  📝 Updated index: {current_id} → {current_path}")
                ...
                for item in rel_items:
                    ...
                    if target_id == 'pahs-compound':
                        ...
                        if target_id in self.index:
                            ...
                            print(f"  🔍 {frontmatter.get('id')}: pahs-compound current={current_url}, correct={correct_url}, match={current_url == correct_url}")
                        else:
                            print(f"  ⚠️  {frontmatter.get('id')}: pahs-compound not in index!")
                    ...
                    if target_id in self.index:
                        ...
                        if 'url' not in item or item['url'] != correct_url:
                            print(f"  🔧 Updating {target_id}: {old_url} → {correct_url}")
