...
class UnifiedPropertyEnhancementService:
    ...
    def _add_optimized_machine_settings(machine_settings: Dict) -> None:
        ...
        settings_config = {
            "powerRange": {"unit": "W", "min": 20.0, "max": 500.0},
            "pulseDuration": {"unit": "ns", "min": 1.0, "max": 1000.0},
            "wavelength": {"unit": "nm", "min": 355.0, "max": 2940.0},
            "spotSize": {"unit": "mm", "min": 0.01, "max": 10.0},
            "repetitionRate": {"unit": "kHz", "min": 1.0, "max": 1000.0},
            "fluenceRange": {"unit": "J/cm²", "min": 0.1, "max": 50.0},
            "scanningSpeed": {"unit": "mm/s", "min": 1.0, "max": 5000.0}
        }
        ...
        for setting_key in setting_order:
            if setting_key in machine_settings:
                ...
                if numeric_value is None:
                    raise ValueError(f"Invalid numeric value for {setting_key}: {value_str} - no defaults allowed")
    ...
    def _add_triple_format_properties(frontmatter_data: Dict) -> None:
        ...
        # Get material name from frontmatter and resolve to canonical form
        material_name_raw = frontmatter_data.get("title", "").strip()
        if not material_name_raw:
            raise ValueError("Material name (title) is required for property enhancement")
        
        # Use material name resolver to get canonical name for materials.yaml lookup
        resolver = get_material_name_resolver()
        material_name = resolver.resolve_canonical_name(material_name_raw)
        if not material_name:
            raise ValueError(f"Material '{material_name_raw}' not found in materials.yaml - fail-fast requires valid material names")
