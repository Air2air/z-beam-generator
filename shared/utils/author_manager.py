...
Canonical implementation — shared across all layers.

Delegates to data.authors.registry as the single source of truth.
"""
...
from data.authors.registry import get_all_authors as registry_get_all_authors
...
def list_authors() -> str:
    ...
    output.extend(
        [
            ...
            "",
            "Usage: python3 run.py --material 'Steel'  # Author resolved from Materials.yaml",
        ]
    )
    return "\n".join(output)
...
def extract_author_info_from_frontmatter_file(
    ...
    try:
        ...
        frontmatter_path = (
            Path("frontmatter/materials") / f"{material_name}-laser-cleaning.md"
        )
...
def get_author_info_for_material(
    ...
    logging.info(
        f"Resolved author.id {material_author_id} for material "
        f"{material_data_or_name.get('name', 'Unknown')}"
    )
    ...
    logging.info(
        f"👤 Using author {author['name']} (ID: {material_author_id}) from Materials.yaml"
    )
