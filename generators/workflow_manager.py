    ...
    for component_type in component_types:
        try:
            ...
            if result.success:
                ...
                print(f"    ✅ Generated ({result.token_count} tokens, {component_time:.1f}s)")
...
def run_batch_generation(
    ...
    for i, material in enumerate(materials, 1):
        ...
        try:
            ...
            batch_results['total_components_generated'] += len(result['components_generated'])
...
def run_interactive_generation(generator: DynamicGenerator, author_info: Dict[str, Any] = None) -> None:
    """
    ...
    while True:
        ...
        # Material selection
        print("\nSelect material:")
        material_input = input("Enter material name (or 'list' to see all, 'quit' to exit): ").strip()
        ...
        if material_input.lower() == 'quit':
            ...
            print(f"❌ Material '{material_input}' not found. Use 'list' to see available materials.")
            continue
        ...
        components_input = input("Enter component names (comma-separated, or 'all'): ").strip()
...
def run_material_generation(
    ...
    if material not in available_materials:
        raise ValueError(f"Material '{material}' not found. Available materials: {len(available_materials)}")
    ...
    if invalid_components:
        raise ValueError(f"Invalid components: {invalid_components}. Available: {available_components}")
