...
class ComponentGeneratorFactory:
    ...
    @classmethod
    def register_generator(cls, component_type: str, generator_class: Type[Any]) -> None:
        """Register a generator class for a component type."""
        cls._generators[component_type] = generator_class
        logger.debug(f"Registered generator for {component_type}: {generator_class.__name__}")
        ...
        try:
            ...
            try:
                ...
                if component_type in components.list_components():
                    ...
                    logger.info(f"Created {component_type} generator from centralized registry")
                    return generator
            except ImportError:
                logger.warning("Centralized component registry not available, falling back to direct imports")
    ...
    @classmethod
    def _create_generator_from_components(cls, component_type: str, **kwargs) -> Optional[Any]:
        ...
        try:
            ...
            generator = generator_class(**kwargs)
            logger.info(f"Created {component_type} generator from components.{component_type}.generator")
            return generator

        except (ImportError, AttributeError) as e:
            logger.debug(f"Could not create {component_type} generator from components: {e}")
            return None

    @classmethod
    def _create_generator_from_generators(cls, component_type: str, **kwargs) -> Optional[Any]:
        ...
        try:
            ...
            generator = generator_class(**kwargs)
            logger.info(f"Created {component_type} generator from generators.{component_type}_generator")
            return generator

        except (ImportError, AttributeError) as e:
            logger.debug(f"Could not create {component_type} generator from generators: {e}")
            return None
