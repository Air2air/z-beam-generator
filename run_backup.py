#!/usr/bin/env python3
"""
Z-Beam content generation system entry point - INTERACTIVE MODE ONLY

This script exclusively uses the interactive generation mode from z_beam_generator.py
for optimal user control and generation quality.

USAGE:
    python3 run.py                    # Start interactive mode from beginning
    python3 run.py --start-from "Copper"  # Start from specific material
    python3 run.py --test-api          # Test API connection only
    python3 run.py --list-materials    # List all available materials

INTERACTIVE MODE FEATURES:
- Step-by-step material processing with user prompts
- Real-time progress tracking and component validation
- Skip, pause, and resume capabilities  
- Comprehensive error handling and recovery
- Session summaries and statistics

For batch processing or other modes, use z_beam_generator.py directly.
"""
...
def main():
    """Main entry point - delegates to z_beam_generator.py interactive mode."""
    ...
    parser = argparse.ArgumentParser(
        description="Z-Beam Interactive Generator",
...
Examples:
  python3 run.py                      # Start interactive mode
  python3 run.py --start-from "Copper" # Resume from Copper
  python3 run.py --test-api           # Test API connection
  python3 run.py --list-materials     # List available materials
        """
    )
    
    parser.add_argument("--start-from", help="Start interactive mode from specific material")
    ...
    if not args.test_api and not args.list_materials:
        print("🎮 Z-Beam Interactive Generator")
        print("="*50)
        print("Starting interactive material generation mode...")
...
BATCH_CONFIG = {
    # Generation mode: "single" for one subject, "multi" for multiple subjects
        "mode": "multi",  # "single", "multi", or "unified"
    ...
    "multi_subject": {
        ...
        "limit": [0,4] # Test with just first 5 materials to verify stability
    },
    ...
    "components": {
        ...
        "bullets": {
            "enabled": False,  # Temporarily disable to reduce complexity
            "count": 4,
...
def get_component_output_path(component_name: str, subject: str, category: str, article_type: str) -> str:
    ...
    safe_category = SlugUtils.create_slug(category)
    safe_article_type = SlugUtils.create_slug(article_type)
...
def prompt_for_autonomous_fix_authorization(subject: str, failed_components: list, error_details: str = None) -> bool:
    ...
    print(f"  • Content quality fixes (placeholder content replacement)")
    print(f"\nThese fixes will:")
    ...
    # Auto-approve for continuous processing
    print("\n✅ AUTO-APPROVING autonomous fixes for continuous processing...")
    response = 'y'  # Automatically approve fixes
    if response in ['y', 'yes']:
        print(f"✅ Authorized: Applying autonomous fixes for {subject}")
        return True
...
def attempt_targeted_autonomous_fixes(subject: str, failed_components: list, category: str = None, article_type: str = None) -> bool:
    """
    Attempts targeted autonomous fixes based on validation_fix_instructions.yaml
    
    Args:
        subject: The material subject being processed
        failed_components: List of component names that failed validation
        category: Category of the subject (if available)
        article_type: Article type (if available)
    
    Returns:
        bool: True if all fixes were successful, False otherwise
    """
    try:
        # Load fix instructions
        instructions_path = "validators/validation_fix_instructions.yaml"
        if not os.path.exists(instructions_path):
            print(f"⚠️ Fix instructions not found at {instructions_path}")
            return False
            
        with open(instructions_path, 'r') as f:
            fix_instructions = yaml.safe_load(f)
        
        # Initialize validator for fix attempts
        from validators.centralized_validator import CentralizedValidator
        validator = CentralizedValidator()
        
        # Determine category and article_type if not provided
        if not category or not article_type:
            # Try to determine from materials.yaml
            ...
            if os.path.exists(yaml_path):
                ...
                for subject_info in subjects_with_info:
                    if subject_info["subject"] == subject:
                        category = category or subject_info["category"]
                        article_type = article_type or subject_info["article_type"]
                        break
            
            # Fallback defaults
            category = category or "material"
            article_type = article_type or "material"
        
        all_fixes_successful = True
        
        for component in failed_components:
            print(f"    🔧 Applying targeted fixes to {component}...")
            
            # Use existing path resolution which handles all slug/filename logic
            component_path = get_component_output_path(component, subject, category, article_type)
            
            if not os.path.exists(component_path):
                print(f"    ⚠️ Component file not found: {component_path}")
                # Try to regenerate only this specific component
                success = regenerate_single_component(subject, component, category, article_type)
                if not success:
            ...
            # Apply targeted fix strategies from instructions
            fix_strategies = fix_instructions.get('fix_approaches', {})
            
            # Try schema compliance fix first
            if 'schema_compliance_fix' in fix_strategies:
                fixed_content = apply_schema_compliance_fix(current_content, component, subject, fix_instructions)
                if fixed_content != current_content:
                    with open(component_path, 'w') as f:
                        f.write(fixed_content)
                    print(f"    ✅ Applied schema compliance fix to {component}")
                    continue
            
            # Try format structure fix
            if 'format_structure_fix' in fix_strategies:
                fixed_content = apply_format_structure_fix(current_content, component, subject, fix_instructions)
                if fixed_content != current_content:
                    with open(component_path, 'w') as f:
                        f.write(fixed_content)
                    print(f"    ✅ Applied format structure fix to {component}")
                    continue
            
            # Try content quality fix
            if 'content_quality_fix' in fix_strategies:
                fixed_content = apply_content_quality_fix(current_content, component, subject, fix_instructions)
                if fixed_content != current_content:
                    with open(component_path, 'w') as f:
                        f.write(fixed_content)
                    print(f"    ✅ Applied content quality fix to {component}")
                    continue
            
            print(f"    ⚠️ No applicable fixes found for {component}")
            all_fixes_successful = False
        
        return all_fixes_successful
        
    except Exception as e:
        print(f"⚠️ Error during autonomous fixing: {e}")
        return False

def apply_schema_compliance_fix(content: str, component: str, subject: str, instructions: dict) -> str:
    """Apply schema compliance fixes based on examples"""
    try:
        example_path = f"validators/examples/{component}.md"
        if os.path.exists(example_path):
            with open(example_path, 'r') as f:
                example_content = f.read()
            
            # Extract required fields from example and add to content if missing
            # This is a simplified implementation - would need component-specific logic
            return content
        return content
    except:
        return content

def apply_format_structure_fix(content: str, component: str, subject: str, instructions: dict) -> str:
    """Apply format structure fixes"""
    try:
        ...
        if component in ['frontmatter', 'metatags', 'jsonld']:
            try:
                yaml.safe_load(content)  # Test if valid YAML
            except yaml.YAMLError:
                # Basic YAML structure repair would go here
                pass
...
def apply_content_quality_fix(content: str, component: str, subject: str, instructions: dict) -> str:
    """DISABLED: No placeholder replacement - force regeneration instead"""
    logger.error(f"❌ CONTENT QUALITY FIX DISABLED for {component} - refusing to apply placeholder replacements")
    logger.error("   Component must be regenerated with real content, not patched with placeholder data")
    return content  # Return unchanged to force regeneration

def regenerate_single_component(subject: str, component: str, category: str, article_type: str) -> bool:
    """Regenerate a single specific component for a subject using existing systems"""
    try:
        print(f"        🔄 Regenerating {component} for {subject}...")
        ...
        # Generate complete document and extract only the component we need
        ...
        if result and isinstance(result, dict) and result.get("success", False):
            ...
            if component in components:
                component_content = components[component]
                if component_content and isinstance(component_content, str) and component_content.strip():
                    # Use existing save function which handles all path/slug logic
                    save_component_output(component, subject, component_content, category, article_type)
                    print(f"        ✅ Successfully regenerated {component}")
                    return True
                else:
                    print(f"        ❌ {component} content is empty or invalid")
                    return False
            else:
                print(f"        ❌ {component} not found in generated components")
                return False
        else:
            print(f"        ❌ Failed to regenerate document - no valid result")
            return False
            
    except Exception as e:
        print(f"        ❌ Error regenerating {component}: {e}")
        return False

def regenerate_missing_component(subject: str, component: str) -> bool:
    """Legacy function - redirects to regenerate_single_component with default values"""
    return regenerate_single_component(subject, component, "material", "material")
...
def run_enhanced_material_processing():
    ...
    # Initialize progress tracking for all materials
    start_progress_session(len(subjects_with_info))
    ...
    for i, subject_info in enumerate(subjects_with_info, 1):
        ...
        # Initialize failure tracking for this material
        has_remaining_failures = False
        
        # Auto-continue to next material processing
        if i > 1:
            print(f"\n✅ AUTO-CONTINUING to next material processing...")
            print(f"Previous material processing complete.")
        ...
        # Start tracking progress for this material
        component_names = [comp for comp, config in BATCH_CONFIG["components"].items() if config.get("enabled", False)]
        start_material_progress(subject, i, component_names)
                ...
                for component in enabled_components:
                    ...
                    if component_content:
                        ...
                        if validation_success:
                            ...
                            # Mark component as completed for progress tracking
                            mark_component_done(component)
                        else:
        ...
        if failed_components:
            ...
            if not authorized:
                ...
                print(f"⏭️ Skipping {subject} - autonomous fixes declined")
                continue
            ...
            if remaining_failed:
                ...
                # Auto-approve additional autonomous fixes
                print(f"\n🔧 AUTONOMOUS FIX OPTION:")
                print(f"   The system can attempt additional targeted fixes based on validation guidelines")
                print(f"   This will apply schema, format, and content quality fixes as appropriate")
                
                print(f"✅ AUTO-APPROVING additional autonomous fixes for {subject}...")
                retry_choice = "y"  # Auto-approve
                
                if retry_choice == "y":
                    print(f"🔄 Attempting additional autonomous fixes for {subject}...")
                    
                    # Implement targeted autonomous fixes based on validation_fix_instructions.yaml
                    success = attempt_targeted_autonomous_fixes(subject, remaining_failed, category, article_type)
                    
                    if success:
                        print(f"✅ Autonomous fixes successful for {subject}")
                        # Re-validate to confirm fixes
                        print(f"🔍 Re-validating {subject} after autonomous fixes...")
                        validation_report = validator.validate_material(subject)
                        
                        # Check if all components are now successful
                        all_components_valid = all(
                            result.status.value == 'success' 
                            for result in validation_report.values()
                        )
                        ...
                        if all_components_valid:
                            ...
                            print(f"⚠️ Some issues remain after autonomous fixes. Manual intervention needed.")
                            break
                    else:
                        print(f"⚠️ Autonomous fixes could not resolve all issues. Manual intervention needed.")
                        break
                else:
                    print(f"⏭️ Skipping autonomous fixes for {subject}")
                    break
                ...
                # Mark material as finished for progress tracking
                finish_material_progress()
        else:
            completed_materials.append(subject)
        ...
        if not has_remaining_failures:
            print(f"✅ Document processing completed for {subject} (components already written)")
            # Note: Components are already written by the unified generator during generation
    ...
    if materials_needing_intervention:
        ...
        # Auto-rerun enhanced cycle for failed materials
        print(f"\n✅ AUTO-RERUNNING enhanced cycle for failed materials...")
        # Pass materials with their specific failed components
        return run_enhanced_material_processing_for_subjects(materials_needing_intervention)
    else:
        print(f"\n🎉 All materials completed successfully!")
    
    # Stop progress tracking before return
    stop_progress_tracking()
    
    return completed_materials

def run_enhanced_material_processing_for_subjects(materials_with_failures: list):
    """Run enhanced processing for specific subjects with their known failed components."""
    print(f"🔄 Rerunning enhanced processing for {len(materials_with_failures)} materials with targeted component regeneration")
    ...
    for i, material_info in enumerate(materials_with_failures, 1):
        subject = material_info['subject']
        failed_components = material_info['failed_components']
        
        print(f"\n{'='*60}")
        print(f"🔄 RETRY {i}/{len(materials_with_failures)}: {subject}")
        print(f"{'='*60}")
        
        # Determine category for this subject (try to get from materials.yaml)
        category = "material"  # Default
        article_type = "material"
        
        # Try to get proper category and article_type from materials.yaml
        yaml_path = os.path.join("lists", "materials.yaml")
        if os.path.exists(yaml_path):
            try:
                subjects_with_info = get_subjects_from_consolidated_yaml(yaml_path)
        ...
        print(f"🔧 Attempting to regenerate {len(failed_components)} failed components for {subject}: {', '.join(failed_components)}")
        
        # Regenerate only the failed components using targeted approach
        regeneration_success = True
        for component in failed_components:
            print(f"    🔄 Regenerating {component}...")
            success = regenerate_single_component(subject, component, category, article_type)
            if not success:
        ...
        if regeneration_success:
            print(f"✅ Successfully regenerated all failed components for {subject}")
            
            # Validate all components after regeneration
            print(f"🔍 Validating all components for {subject}...")
            validation_report = validator.validate_material(subject)
            
            # Check results
            failed_after_regen = [
                comp for comp, result in validation_report.items() 
                if result.status.value != 'success'
            ]
            
            if failed_after_regen:
                print(f"⚠️ {subject} still has {len(failed_after_regen)} failed components after regeneration: {', '.join(failed_after_regen)}")
                still_failing.append(subject)
            else:
                print(f"🎉 {subject} completed successfully after targeted regeneration!")
                completed_materials.append(subject)
        else:
            print(f"❌ Failed to regenerate some components for {subject}")
            still_failing.append(subject)
...
def apply_validation_fixes(processed_subjects: list) -> None:
    ...
    try:
        ...
        print("❌ Unable to proceed with validation fixes. Manual intervention required.")
...
# DEAD CODE - LEGACY VALIDATION SYSTEM (DISABLED)
# def apply_validation_fixes_legacy(processed_subjects: list) -> None:
    ...
    # instructions = load_validation_fix_instructions()  # DISABLED
