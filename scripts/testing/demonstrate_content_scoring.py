...
def demonstrate_content_scoring():
    ...
    for i, author in enumerate(authors, 1):
        ...
        try:
            ...
            if result.success:
                ...
                print(
                    f"📏 Content Length: {metadata.get('word_count', len(result.content.split()))} words"
                )
                print(
                ...
                if result.quality_score:
                    ...
                    print(
                        f"  👤 Human Believability: {score.human_believability:.1f}/100"
                    )
                    print(f"  🔬 Technical Accuracy: {score.technical_accuracy:.1f}/100")
                    print(
                        f"  ✍️  Author Authenticity: {score.author_authenticity:.1f}/100"
                    )
                    ...
                    print(
                        f"  📏 Avg Sentence Length: {score.avg_sentence_length:.1f} words"
                    )
                    ...
                    print(f"\n✅ VALIDATION STATUS:")
                    print(
                        f"  🎯 Required Elements: {'✅' if score.has_required_elements else '❌'}"
                    )
                    print(
                        f"  👤 Human Threshold: {'✅' if score.passes_human_threshold else '❌'} ({score.human_believability:.1f} >= 75.0)"
                    )
                    print(
                        f"  🔄 Retry Recommended: {'❌ Yes' if score.retry_recommended else '✅ No'}"
                    )
                    ...
                    if hasattr(score, "scoring_breakdown") and score.scoring_breakdown:
                        ...
                        # Formatting details
                        fmt_details = score.scoring_breakdown.get("formatting", {}).get(
                            "details", {}
                        )
                        if fmt_details:
                            print(f"  📝 Formatting:")
                            print(
                                f"    - Title: {'✅' if fmt_details.get('has_title') else '❌'}"
                            )
                            print(
                                f"    - Sections: {'✅' if fmt_details.get('has_sections') else '❌'}"
                            )
                            print(
                                f"    - Bold Text: {'✅' if fmt_details.get('has_bold_text') else '❌'}"
                            )
                            print(
                                f"    - Author Byline: {'✅' if fmt_details.get('has_author_byline') else '❌'}"
                            )

                        # Technical details
                        tech_details = score.scoring_breakdown.get(
                            "technical_accuracy", {}
                        ).get("details", {})
                        if tech_details:
                            print(f"  🔬 Technical:")
                            print(
                                f"    - Formula Present: {'✅' if tech_details.get('formula_present') else '❌'}"
                            )
                            print(
                                f"    - Technical Terms: {tech_details.get('technical_term_count', 0)}"
                            )
                            print(
                                f"    - Technical Density: {tech_details.get('technical_density', 0):.3f}"
                            )

                        # Authenticity details
                        auth_details = score.scoring_breakdown.get(
                            "author_authenticity", {}
                        ).get("details", {})
                        if auth_details:
                            print(f"  ✍️  Authenticity:")
                            print(
                                f"    - Author Name: {'✅' if auth_details.get('author_name_present') else '❌'}"
                            )
                            print(
                                f"    - Country: {'✅' if auth_details.get('country_present') else '❌'}"
                            )
                            found_markers = auth_details.get(
                                "found_linguistic_markers", []
                            )
                            print(
                                f"    - Linguistic Markers: {len(found_markers)} found"
                            )
                            if found_markers:
                                print(
                                    f"      {', '.join(found_markers[:3])}{'...' if len(found_markers) > 3 else ''}"
                                )
    ...
    if all_results:
        ...
        if all_results[0].quality_score:
            # Average scores
            overall_scores = [
                r.quality_score.overall_score for r in all_results if r.quality_score
            ]
            human_scores = [
                r.quality_score.human_believability
                for r in all_results
                if r.quality_score
            ]
            tech_scores = [
                r.quality_score.technical_accuracy
                for r in all_results
                if r.quality_score
            ]
            ...
            print(
                f"  👤 Human Believability: {sum(human_scores)/len(human_scores):.1f}/100"
            )
            print(
                f"  🔬 Technical Accuracy: {sum(tech_scores)/len(tech_scores):.1f}/100"
            )
            ...
            human_passes = sum(
                1
                for r in all_results
                if r.quality_score and r.quality_score.passes_human_threshold
            )
            print(f"\n✅ PASS RATES:")
            print(
                f"  👤 Human Threshold (≥75): {human_passes}/{len(all_results)} ({human_passes/len(all_results)*100:.1f}%)"
            )

            # Content metrics
            word_counts = [
                r.quality_score.word_count for r in all_results if r.quality_score
            ]
            print(f"\n📏 CONTENT METRICS:")
        ...
        print(
            f"🚀 All {successful_generations} authors producing scored, human-believable content!"
        )
