#!/usr/bin/env python3
"""
Script Name: Content Generator Cleanup
Description: Archives unused generator files and keeps only the production fail-fast generator
Usage: python3 scripts/cleanup/cleanup_generators.py
Dependencies: shutil, datetime, pathlib
...
def cleanup_generators():
    ...
    if len(remaining_generators) == 1 and "fail_fast" in remaining_generators[0].name:
        ...
        print(
            f"⚠️  Unexpected generators remain: {[g.name for g in remaining_generators]}"
        )
        return False
