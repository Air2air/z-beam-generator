    ...
    try:
        ...
        for author in authors:
            ...
            if result.success:
                ...
                has_professional_tone = any(
                    word in content.lower()
                    for word in ["systematic", "analysis", "optimization", "parameters"]
                )
                has_markdown_formatting = "##" in content and "**" in content
                has_specific_details = any(
                    phrase in content
                    for phrase in ["laser", "cleaning", "surface", "material"]
                )
                ...
                print(
                    f"  Author {author['id']} ({author['country']}): {believability_score}/5 believability factors"
                )
        ...
        print(
            f"📊 Result: {believable_count}/{len(authors)} authors produce believable content"
        )
        return success
...
def check_requirement_2_no_mocks_fallbacks():
    ...
    try:
        ...
        print(
            f"📊 Result: {'✅ No fallbacks detected' if success else '❌ Fallbacks or missing error handling found'}"
        )
        return success
...
def check_requirement_3_formatting_personas():
    ...
    try:
        ...
        avg_formatting_size = (
            sum(formatting_sizes) / len(formatting_sizes) if formatting_sizes else 0
        )
        ...
        success = (
            formatting_exists
            and persona_exists
            and loads_formatting
            and loads_personas
            and avg_formatting_size > 1000
        )
        print(
            f"📊 Result: {'✅ Formatting and personas properly integrated' if success else '❌ Missing or inadequate formatting/persona integration'}"
        )
        return success
...
def check_requirement_4_frontmatter_grok():
    ...
    try:
        ...
        frontmatter_data = {
            ...
            "laser_cleaning": {"wavelength": "1064nm", "pulse_duration": "10ns"},
        }
        ...
        print(
            f"  Generation method: {result.metadata.get('generation_method', 'unknown')}"
        )

        success = api_required and result.success
        print(
            f"📊 Result: {'✅ Proper frontmatter and API integration' if success else '❌ Missing frontmatter or API integration'}"
        )
        return success
...
def check_requirement_5_validation_retries():
    ...
    try:
        ...
        has_validation = any(
            term in content for term in ["validate", "validation", "check"]
        )
        ...
        print(
            f"📊 Result: {'✅ Proper validation and retry system' if success else '❌ Missing or incomplete validation/retry system'}"
        )
        return success
...
def check_requirement_6_e2e_evaluation():
    ...
    try:
        ...
        print(
            f"  Content generated: {len(result.content) if result.success else 0} chars"
        )
        ...
        print(
            f"📊 Result: {'✅ E2E system effective' if success else '❌ E2E system needs improvement'}"
        )
