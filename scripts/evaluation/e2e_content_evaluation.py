...
class ContentGenerationEvaluator:
    ...
    def _evaluate_persona_authenticity(self, content: str, author: Dict) -> Dict:
        ...
        style_matches = sum(
            1 for indicator in style_indicators if indicator.lower() in content.lower()
        )
    ...
    def _evaluate_language_patterns(self, content: str, country: str) -> int:
        ...
        if country == "taiwan":
            ...
            matches = sum(
                1 for pattern in mandarin_patterns if pattern in content.lower()
            )
            ...
            matches = sum(
                1 for pattern in italian_patterns if pattern in content.lower()
            )
            score = min(10, matches * 3)
    ...
    def _calculate_believability_score(
        self, content: str, validation_result: Dict, material: Dict
    ) -> int:
        ...
        final_score = int(
            base_score * 0.4
            + technical_accuracy * 0.3  # 40% human-likeness
            + natural_flow * 0.2  # 30% technical accuracy
            + authenticity * 0.1  # 20% natural flow  # 10% authenticity markers
        )
    ...
    def _check_authenticity_markers(self, content: str) -> int:
        ...
        marker_count = sum(
            1 for marker in personal_markers if marker.lower() in content.lower()
        )
        ...
        transition_count = sum(
            1 for trans in transitions if trans.lower() in content.lower()
        )
    ...
    def generate_evaluation_report(self):
        ...
        if authenticity_results:
            avg_authenticity = sum(
                r["authenticity_score"]["total"] for r in authenticity_results
            ) / len(authenticity_results)
            print(
                f"🎭 AUTHENTICITY: {'✅ EXCELLENT' if avg_authenticity > 80 else '⚠️ NEEDS IMPROVEMENT'}"
            )
            print(f"   - Average Score: {avg_authenticity:.1f}/100")

        # Believability Summary
        if believability_results:
            avg_believability = sum(
                r["believability_score"] for r in believability_results
            ) / len(believability_results)
            print(
                f"🧠 BELIEVABILITY: {'✅ EXCELLENT' if avg_believability > 85 else '⚠️ NEEDS IMPROVEMENT'}"
            )
