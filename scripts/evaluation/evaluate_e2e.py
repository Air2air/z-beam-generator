...
def test_fail_fast_generator(material):
    ...
    try:
        ...
        return {
            ...
            "file_size": get_generator_file_size(
...
def print_generator_results(name, result):
    ...
    if result.get("success"):
        print(
            f"✅ Success: {result['content_length']} chars in {result['generation_time']:.3f}s"
        )
        print(
            f"📁 File size: {result['file_size']['lines']} lines, {result['file_size']['kb']} KB"
        )
...
def compare_generators(results):
    ...
    print(
        f"🎯 Fewest dependencies: {fewest_deps[0]} ({len(fewest_deps[1]['dependencies'])})"
    )
    print(
        f"🕸️  Most dependencies: {most_deps[0]} ({len(most_deps[1]['dependencies'])})"
    )
    ...
    for gen_path in generators:
        if Path(gen_path).exists():
            ...
            uses_formatting = (
                ...
                or "_formatting.yaml" in content
                or "load_formatting" in content
            )
...
def check_persona_usage():
    ...
    for gen_path in generators:
        if Path(gen_path).exists():
            ...
            uses_personas = (
                "personas/" in content
                or "_persona.yaml" in content
                or "load_persona" in content
            )
...
def check_mock_usage():
    ...
    mock_patterns = ["mock", "fallback", "default_content", "hardcoded", "placeholder"]
...
def provide_recommendations(results):
    ...
    print("\n📋 FORMATTING FILES:")
    formatting_files = list(
        Path("components/content/prompts/formatting").glob("*.yaml")
    )
