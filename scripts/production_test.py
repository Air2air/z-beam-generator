    ...
    try:
        ...
        for i, author in enumerate(authors, 1):
            ...
            if result.success:
                ...
                print(
                    f"    📊 Method: {result.metadata.get('generation_method', 'unknown')}"
                )
                ...
                has_technical = any(
                    ...
                    for term in ["laser", "cleaning", "surface"]
                )

                quality_score = sum(
                    [has_title, has_formula, has_markdown, has_technical]
                )
        ...
        for i, result in enumerate(all_results, 1):
            ...
            found_fallbacks = [
                indicator for indicator in fallback_indicators if indicator in content
            ]

            if found_fallbacks:
                print(
                    f"    ❌ Author {i} content contains fallback indicators: {found_fallbacks}"
                )
                return False
            else:
        ...
        print(
            ...
            sum(len(r.content) for r in all_results) // len(all_results),
            "characters",
        )
