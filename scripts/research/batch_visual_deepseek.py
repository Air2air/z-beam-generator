#!/usr/bin/env python3
"""
Super-Batched Visual Appearance Research using DeepSeek

Batches ALL 13 categories into a single API call per pattern.
Result: 100 API calls instead of 1,298 (92% reduction)

Usage:
    python3 scripts/research/batch_visual_deepseek.py --all
    python3 scripts/research/batch_visual_deepseek.py --pattern rust-oxidation
    python3 scripts/research/batch_visual_deepseek.py --all --dry-run
"""
...
CATEGORY_EXAMPLES = {
    ...
    'mineral': 'ite minerals, calcium carbide, Silica',
...
class DeepSeekBatchResearcher:
    ...
    def __init__(self, rate_limit: int = 30):
        ...
        print("🚀 Super-Batch Researcher initialized (DeepSeek)")
    ...
    def build_super_prompt(self, pattern_name: str, categories: list) -> str:
        ...
        Each field should be 1-2 short sentences since they get concatenated
        with other prompt components for Imagen.
        """
...
3. coverage: Typical coverage amount and variation
    ...
    def research_pattern(self, pattern_id: str, pattern_name: str, categories: list) -> dict:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                temperature=0.5,
                max_tokens=4000,  # ~150 chars/category × 13 = ~2000 chars needed
...
def main():
    parser = argparse.ArgumentParser(description='Super-Batch Visual Appearance Research (DeepSeek)')
    ...
    parser.add_argument('--rate', type=int, default=30, help='Requests per minute (default: 30)')
    ...
    researcher = DeepSeekBatchResearcher(rate_limit=args.rate)
