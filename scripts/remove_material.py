...
class MaterialRemover:
    ...
    def save_materials(self, materials_data: Dict, backup: bool = True) -> None:
        ...
        full_data = {
            ...
            "metadata": {
                ...
                "total_materials": sum(
                    len(cat.get("items", [])) for cat in materials_data.values()
                ),
    ...
    def find_material_files(self, material_name: str) -> Dict[str, List[Path]]:
        ...
        filename_base = create_filename_slug(material_name).replace(
            "-laser-cleaning", ""
        )
    ...
    def remove_material(
        self, material_name: str, dry_run: bool = True
    ) -> Dict[str, any]:
        ...
        if material_location:
            ...
            logger.info(
                f"Found material '{material_name}' in category '{category}' at index {index}"
            )
        else:
            logger.warning(f"Material '{material_name}' not found in materials list")
            results["errors"].append(
                f"Material '{material_name}' not found in materials list"
            )
        ...
        if material_files:
            ...
            logger.info(
                f"Found {total_files} files across {len(material_files)} component types"
            )
...
EXAMPLES:
  ...
  python3 remove_material.py --material "Aluminum" --execute --confirm
    ...
    parser.add_argument(
        "--execute", action="store_true", help="Actually perform the removal operations"
    )
    parser.add_argument(
        "--confirm", action="store_true", help="Require confirmation before removal"
    )
    parser.add_argument(
        "--list-materials",
        action="store_true",
    ...
    if args.material:
        ...
        if results["material_found"]:
            logger.info(
                f"✅ Material found in category: {results.get('material_category', 'unknown')}"
            )
