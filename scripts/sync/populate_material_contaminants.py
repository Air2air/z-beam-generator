...
- "What contaminants affect Aluminum?" → O(1) cached lookup
- "What materials can have rust?" → O(1) direct lookup
...
def populate_materials(
    ...
    Args:
        ...
        contaminant_index: Material → Contaminants mapping
    ...
    for material_name, material_data in materials.items():
        ...
        if not valid_contaminants and not prohibited_contaminants:
            if verbose:
                print(f"   ⏭️  {material_name}: No contamination data")
            skipped_count += 1
            continue
...
"""
    ...
    if not contaminants_file.exists():
        print(f"❌ Error: {contaminants_file} not found")
        sys.exit(1)
    ...
    print('=' * 80)
    print('🔄 MATERIAL CONTAMINANTS SYNC (HYBRID APPROACH)')
    print('=' * 80)
    print()
    print('Architecture:')
    ...
    print(f'   ✅ Loaded {patterns_count} contamination patterns')
    print(f'   ✅ Loaded {materials_count} materials')
    print()
    
    # Build reverse index
    print('🔍 Building material → contaminants index...')
    ...
    print(f'   ✅ {materials_with_data} materials have contamination data')
    print(f'   ✅ {total_associations} total associations')
    print()
    
    # Show filter if specific materials requested
    if args.materials:
        print(f'🎯 Updating specific materials: {", ".join(args.materials)}')
        print()
    ...
    if updated_count > 0 and not args.verbose:
        ...
        for material_name, material_data in updated_data.get('materials', {}).items():
            ...
            if contaminants and example_count < 5:
                print(f'   {material_name}:')
                print(f'      → {len(contaminants)} contaminants: {", ".join(contaminants[:3])}' +
