#!/usr/bin/env python3
"""
Batch Caption Generator - Generate micros for all materials missing them

This script generates microscopy micros for all materials in Materials.yaml
that are missing beforeText or afterText caption fields.

Usage:
    # Generate all missing micros
    python3 scripts/batch/generate_all_micros.py
    
    # Dry run (show what would be generated)
    python3 scripts/batch/generate_all_micros.py --dry-run
    
    # Generate for specific materials only
    python3 scripts/batch/generate_all_micros.py --materials "Bronze,Steel,Aluminum"
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch generate micros for materials missing them'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    if args.materials:
        ...
        logger.info(f"Found {len(material_names)} materials missing micros")
    ...
    if args.dry_run:
        logger.info("🔍 DRY-RUN MODE: No micros will be generated")
        logger.info(f"\nWould generate micros for {len(material_names)} materials:")
    ...
    logger.info(f"\n🚀 Generating micros for {len(material_names)} materials...")
    logger.info("=" * 60)
