...
class TestCleanupSystem(unittest.TestCase):
    ...
    def test_generate_cleanup_report(self):
        ...
        print(f"    Unused component files: {len(report.get('unused_component_files', []))}")
        ...
        total_issues = sum(len(report.get(key, [])) for key in report.keys() 
...
class CleanupManager:
    ...
    def _load_cleanup_config(self) -> dict:
        ...
        return {
            ...
            'empty_dir_patterns': [
                # Common patterns for directories that should be empty or removed
                'tmp/',
                'temp/',
                'cache/',
                'logs/',
    ...
    def _find_dead_files(self) -> List[Tuple[str, str]]:
        ...
        if components_dir.exists() and content_dir.exists():
            ...
            component_types = {d.name for d in components_dir.iterdir() 
                    ...
                    try:
                        with open(config_path, 'r') as f:
                            ...
                            for component_dir in components_dir.iterdir():
                                if component_dir.is_dir() and component_dir.name in content:
            ...
            if dir_path.is_dir() and not self._is_excluded(dir_path):
                try:
                    ...
                    if not contents:
                        ...
                        # Check if directory only contains hidden files or system files
                        visible_contents = [
                            item for item in contents 
                            if not item.name.startswith('.') 
                            and item.name not in ['__pycache__', 'Thumbs.db', 'desktop.ini']
                        ]
                        
                        if not visible_contents:
                            empty_dirs.append((
                                str(dir_path.relative_to(self.project_root)),
                                f"Only hidden/system files: {[item.name for item in contents]}"
                            ))
                            
                        # Check for directories with only empty subdirectories
                        elif all(item.is_dir() for item in visible_contents):
                            # Check if all subdirectories are empty
                            all_subdirs_empty = True
                            for subdir in visible_contents:
                                if any(subdir.rglob('*')):
                            ...
                            if all_subdirs_empty:
                                empty_dirs.append((
                                    str(dir_path.relative_to(self.project_root)),
                                    f"Contains only empty subdirectories: {[item.name for item in visible_contents]}"
                                ))
    ...
    def _is_excluded(self, path: Path) -> bool:
        ...
        # Don't exclude important project directories even if they're empty
        important_dirs = [
            'components',
            'content',
            'schemas',
        ...
        # Check if this is a top-level important directory
        path_parts = Path(path_str).parts
        if path_parts and path_parts[0] in important_dirs:
            # Allow important directories to be checked for emptiness
            return False
...
TestCleanupSystem._find_dead_files = lambda self: CleanupManager(self.project_root)._find_dead_files()
TestCleanupSystem._find_unused_component_files = lambda self: CleanupManager(self.project_root)._find_unused_component_files()
TestCleanupSystem._find_temporary_files = lambda self: CleanupManager(self.project_root)._find_temporary_files()
TestCleanupSystem._find_empty_directories = lambda self: CleanupManager(self.project_root)._find_empty_directories()
TestCleanupSystem._find_outdated_generated_content = lambda self: CleanupManager(self.project_root)._find_outdated_generated_content()
TestCleanupSystem._find_broken_symlinks = lambda self: CleanupManager(self.project_root)._find_broken_symlinks()
TestCleanupSystem._generate_cleanup_report = lambda self: CleanupManager(self.project_root).run_cleanup()
