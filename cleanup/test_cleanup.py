...
class TestCleanupSystem(unittest.TestCase):
    ...
    def test_generate_cleanup_report(self):
        ...
        print(f"    Misplaced test files: {len(report.get('misplaced_test_files', []))}")
...
TestCleanupSystem._find_misplaced_test_files = lambda self: CleanupManager(self.project_root)._find_misplaced_test_files()
TestCleanupSystem._find_misplaced_doc_files = lambda self: CleanupManager(self.project_root)._find_misplaced_doc_files()
