...
A simpler recovery system that calls generators directly without modifying run.py.
...
def main():
    """CLI interface for direct recovery"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Direct Component Recovery")
    parser.add_argument('subject', help='Subject to recover')
    parser.add_argument('--components', nargs='+', required=True, 
                       help='Components to recover (e.g., frontmatter metatags)')
    parser.add_argument('--article-type', default='material', 
                       help='Article type (default: material)')
    parser.add_argument('--category', help='Category (auto-detected if not provided)')
    parser.add_argument('--author-id', type=int, default=1, help='Author ID (default: 1)')
    parser.add_argument('--timeout', type=int, default=60, help='Timeout per component')
    ...
    results = runner.recover_components(
        ...
        failed_components=args.components,
